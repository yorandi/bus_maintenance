    ...
    <div class="card-body table-responsive">
        <table class="table table-hover align-middle">
            ...
            <tbody>
                @forelse($inspections as $inspection)
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted">{{ $inspection->vehicle->merk }} {{ $inspection->vehicle->model }}</small>
                        </td>
                        <td>{{ $inspection->driver->name }}</td>
                        <td>{{ number_format($inspection->odometer, 0, ',', '.') }} km</td>
                        <td>
                            @php
                                $badge = ['siap_operasi' => 'success', 'perlu_perbaikan' => 'warning', 'tidak_layak' => 'danger'][$inspection->condition_result] ?? 'secondary';
                            @endphp
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">Belum ada data pemeriksaan operasional</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
